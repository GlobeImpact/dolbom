    ...
    <thead>
        <tr>
            <th colspan="<?php echo $cols_count ?>" style="border:1px solid #000; height:40px;">
                <p style="text-align:center; vertical-align:middle;">[<?php echo $branch_row['branch_name'] ?>] [<?php echo $mn_row['me_name'] ?>] 자격정보관리 리스트</p>
            </th>
        </tr>
        <?php echo $list['hd'] ?>
    </thead>
            ...
            <?php
                if($set_num2 > 0) {
                    for($m=0; $set_row2 = sql_fetch_array($set_qry2); $m++) {
                        ...
                        echo '<div>진단일자 : '.$list['list'][$i]['qualification'][$m]['diagnosis_date'].'</div>';
                        echo '<div>판정일자 : '.$list['list'][$i]['qualification'][$m]['judgment_date'].'</div>';
                        echo '<div>확인일자 : '.$list['list'][$i]['qualification'][$m]['confirm_date'].'</div>';
